...
class RecordData(NamedTuple):
    ...
    def match(self, rtype:Optional[str]=None, name:Optional[str]=None, content:Optional[str]=None)->bool:
        return (rtype == None or rtype.lower() == self.rtype.lower()) and (name == None or name.lower() == self.name.lower()) and (content == None or content.lower() == self.content.lower())
...
def vdapi_build_caller(opener:OpenerDirector, content_type:str, headers:Optional[Dict[str,str]] = None,  content_decoder:Callable[[T],bytes] = lambda x: x) -> RESTAPI_CALLER_TYPE:    
    def _(url:str, method:str, content:Optional[T] = None, interval=1) -> Tuple[HTTPResponse, bytes]:
        try:
            ...
            if http_error.status == 429: # Too much Request
                time.sleep(interval)
...
def vdapi_get_domain_data(caller:RESTAPI_CALLER_TYPE, domainname:str)->Optional[DomainData]:    
    resp:RestApiResponse = caller(f'{VDAPI_ENDPOINT}/domains/{domainname}/dns', 'GET')        
    restapi_exception_not_200(resp.header)
    domain_info:dict = json.loads(resp.data.decode('utf-8').strip())
    domain_records:Optional[str] = domain_info.get('results',{}).get('records')     
    return DomainData(
        [ RecordData(elem[0], elem[1], elem[2]) for elem in [ line.split(' ', 3) for line in domain_records.split('\n') ] if len(elem) == 3 ],
        int(domain_info.get('results',{}).get('ttl', DEFAULT_TTL))
     ) if domain_records != None else None
...
class Provider(BaseProvider):
    ...
    def __init__(self, config):
        ...
        self.caller:Optional[RESTAPI_CALLER_TYPE] = None
    ...
    def _authenticate(self):
        ...
        assert auth_token != None, "No authenticaion token defined"
                
        self.caller = vdapi_create_caller(auth_token)
    ...
    def _list_records(self, rtype:Optional[str]=None, name:Optional[str]=None, content:Optional[str]=None):
        ...
        return [ {
            ...
            } for record_data in domain_data.records if record_data.match(rtype, self._relative_name(name) if name != None else None, content)
        ] if domain_data != None else []
    ...
    def _delete_record(self, identifier=None, rtype=None, name=None, content=None):
        self._assert_initialized()
        domain_data = vdapi_get_domain_data(self.caller, self.domain)
        if domain_data != None and identifier != None:
            vdapi_set_domain_data(self.caller, self.domain, DomainData( [ record for record in domain_data.records if record.id() != identifier ], domain_data.ttl))
        elif domain_data != None:
            vdapi_set_domain_data(self.caller, self.domain, DomainData( [ record for record in domain_data.records if not record.match(rtype, self._relative_name(name), content) ], domain_data.ttl))
...
#        return self.caller(url, action, data)        
    ...
    def _assert_initialized(self):
        if self.caller == None or self.domain_id == None:
            self._authenticate()

        assert self.caller != None, "HTTP caller not defined"
        assert self.domain_id != None or len(self.domain_id == 0), "Domain name not retriebed"
    ...
    class TestValueDomain(unittest.TestCase):
        ...
        def test_vdapi_set_record_list(self):            
            ...
            for domainname in domain_list:
                ...
                vdapi_set_domain_data(self.caller, domainname, DomainData(domain_data.records + [ dummy ], domain_data.ttl))
        ...
        def test_create_records(self):
            ...
            for domainname in domain_list:
                ...
                records = provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT)
                self.assertGreater(len(records), 0)

                provider._create_record(DUMMY_TYPE, f"{DUMMY_NAME}.{domainname}.", DUMMY_CONTENT)
                self.assertEqual(len(provider._list_records(rtype=DUMMY_TYPE, name=f"DUMMY_NAME.{domainname}.", content=DUMMY_CONTENT)), 0)
                
                # cleanup
                provider._delete_record(None, DUMMY_TYPE, DUMMY_NAME, DUMMY_CONTENT)
                self.assertEqual(len(provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT)), 0)
        ...
        def test_delete_records_by_id(self):
            ...
            for domainname in domain_list:
                ...
                provider._create_record(DUMMY_TYPE, DUMMY_NAME, DUMMY_CONTENT)                
                recl = provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT)
        ...
        def test_update_record(self):
            ...
            for domainname in domain_list:                
                ...
                recl = provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT)
                self.assertGreater(len(recl), 0)
                provider._update_record(identifier=recl[0].get('id'), content=DUMMY_CONTENT2)
                self.assertGreater(len(provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT2)), 0)

                # cleanup
                provider._delete_record(None, DUMMY_TYPE, DUMMY_NAME, DUMMY_CONTENT2)
                self.assertEqual(len(provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT)), 0)  
                self.assertEqual(len(provider._list_records(rtype=DUMMY_TYPE, name=DUMMY_NAME, content=DUMMY_CONTENT2)), 0)  
