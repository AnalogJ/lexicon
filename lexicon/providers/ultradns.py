...
class Provider(BaseProvider):
    ...
    def __init__(self, config):
        ...
        assert (self.api_key is not None) or \
            (self._get_provider_option('auth_username') is not None and \
                 self._get_provider_option('auth_password') is not None)
    ...
    def _list_records(self, rtype=None, name=None, content=None):
        ...
        for rrset in self.zone_data()['rrSets']:
            if (name is None or self._fqdn_name(rrset['ownerName']) == self._fqdn_name(
                    name)) and (rtype is None or rrset['rrtype'] == rtype):
                for record in rrset['rdata']:
                    if content is None or record == self._clean_content(rtype, content):
                        records.append({
                            ...
                            'id': self._make_identifier(rrset['rrtype'],
                                                        rrset['ownerName'], record)
                        })
    ...
    def _create_record(self, rtype, name, content):
        ...
        if found:
            self._put('/zones/{0}/rrsets/{1}/{2}'.\
                          format(self._ensure_dot(self.domain), rtype, rname),\
                          data=updated_data)
        else:
            self._post('/zones/{0}/rrsets/{1}/{2}'.\
                           format(self._ensure_dot(self.domain), rtype, rname),\
                           data=updated_data)
    ...
    def _delete_record(self, identifier=None, rtype=None, name=None, content=None):
        ...
        if identifier is not None:
            ...
            for rrset in data['rrSets']:
                for record in rrset['rdata']:
                    ident = self._make_identifier(rrset['rrtype'], rrset['ownerName'], record)
                    if identifier == ident:
                        rtype = rrset['rrtype']
                        name = self._full_name(rrset['ownerName'])
                        content = self._unclean_content(rrset['rrtype'], record)
        ...
        found = False
        for rrset in data['rrSets']:
            if rrset['rrtype'] == rtype and self._fqdn_name(rrset['ownerName']) == rname:
                update_data = rrset
                found = True
        ...
        if update_data['rdata']:
            ...
            self._delete('/zones/{0}/rrsets/{1}/{2}'.\
                             format(self._ensure_dot(self.domain), rtype, rname))
    ...
    def _request(self, action='GET', url='/', data=None, query_params=None):
        ...
        if query_params is None:
            query_params = {}
        response = requests.request(action, "https://restapi.ultradns.com/v2" + url,
                                    params=query_params,
