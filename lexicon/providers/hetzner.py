...
#   * raw  - concatenation of the record type, name(FQDN) and content(if possible FQDN) with delimiter '/'; 
...
def ProviderParser(subparser):
    ...
    subparser.add_argument('--concatenate', 
                        ...
                        'Restriction: Only enabled if the record name or the raw FQDN record identifier \'type/name/content\' '
                        'is spezified, and additionally for update action the record name remains the same',
...
class Provider(BaseProvider):
    ...
    def _request(self, action='GET',  url='/', data=None, query_params=None):
        ...
        for i in range(10):
            try:
                r = self.session.request(action, self.api_endpoint + url, params=query_params, data=data)
    ...
    def _open_session(self, username, password):
        ...
        r = session.request('POST','{0}/login_check'.format(self.auth_endpoint), data={'_username': username, '_password': password})
        if '{0}/account/masterdata'.format(self.auth_endpoint) == r.url and r.status_code == 200:
            r = session.request('GET','{0}/'.format(self.api_endpoint))
        if self.api_endpoint not in r.url or r.status_code != 200:
    ...
    def _close_session(self):
        if self._get_provider_option('live_tests') is None:
            r = self._get('/login/logout/r/true')
            if '{0}/logout'.format(self.auth_endpoint) not in r.url and r.status_code != 200:
                logger.error('Hetzner => Unable to safely close session')
            else:
                logger.info('Hetzner => Close session')
            self.session = None

    def _extract_zone_id_from_js(self, s):
        r = re.compile(r'\'(\d+)\'')
        m = r.search(s)
    ...
    def _get_zone_id(self, zone):
        zone = dns.name.from_text(zone).to_unicode(True)
        id, zones, last_count, page = None, {}, -1, 1
        while (last_count != len(zones) and id is None):
            last_count = len(zones)
            r = self._get('/dns/index/page/{0}'.format(page))
            soup = BeautifulSoup(r.text, 'html.parser')
            boxes = soup.findAll('table', attrs={'class': 'box_title'})
            for box in boxes:
                expandBoxJS = dict(box.attrs)['onclick']
                zone_id = self._extract_zone_id_from_js(expandBoxJS)
        ...
        if id is None:
            ...
            self._close_session()
            raise AssertionError
        logger.info('Hetzner => Get ID {1} for zone {0}'.format(zone, id))
        return id

    def _get_zone(self, zone_name, zone_id):
        r = self._get('/dns/update/id/{0}'.format(zone_id))
        soup = BeautifulSoup(r.text, 'html.parser')
    ...
    def _get_language(self):
        r = self._get('/preferences/culture')
        soup = BeautifulSoup(r.text, 'html.parser')
    ...
    def _post_zone(self):
        ...
        r = self._post('/dns/update', data={'id': self.domain_id, 'zonefile': self.zone['data'].to_text(relativize=True), '_csrf_token': self.zone['token']})
        # ugly: the Hetzner Robot status code is always 200 (delivering the update form as an 'error message')
        if post_response[language] in r.text:
